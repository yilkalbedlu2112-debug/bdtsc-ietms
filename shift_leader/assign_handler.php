<?php
/**
 * POST handler for task assignment from assign_task_view.php (same rules as assign_task.php).
 */
...
function sl_assign_view_redirect(string $type, string $message = ''): void
{
    $q = ['flash' => $type];
    if ($message !== '') {
        $q['msg'] = $message;
...
if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
    sl_assign_view_redirect('error', 'Invalid request method.');
}

if (!$hasAssignedToDept || $dept_id <= 0) {
    sl_assign_view_redirect('error', 'Assignment requires your department and tasks.assigned_to_dept on each task.');
}
...
if (!$task_id || !$employee_id) {
    sl_assign_view_redirect('error', 'Invalid task or employee selection.');
}
...
try {
    ...
    if (!$taskRow || !in_array($taskRow['status'], ['Pending', 'Redo'], true)) {
        $pdo->rollBack();
        sl_assign_view_redirect('error', 'Task is not Pending/Redo for your department, or is not visible.');
    }
    ...
    if (!$empRow) {
        $pdo->rollBack();
        sl_assign_view_redirect('error', 'Selected user is not an active Employee in your department.');
    }
    ...
    $pdo->commit();
    sl_assign_view_redirect('success');
} catch (Throwable $e) {
    if ($pdo->inTransaction()) {
        $pdo->rollBack();
    }
    sl_assign_view_redirect('error', $e->getMessage());
}
